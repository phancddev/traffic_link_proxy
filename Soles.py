...
global checked_proxies_unsuccessful
global finished
global successful_request_list
successful_request_list = []
finished = False
checked_proxies = 0
checked_proxies_unsuccessful = 0
used_proxies = []
successful_requests = 0
country_specific_proxies = []
specific_countries_proxies = 0
...
def setup_logging():
    ...
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s", 
                        handlers=[logging.FileHandler(f"{logDir}/logs.txt", encoding='utf-8'), 
...
def get_country_info(ip):
    ...
    try:
        ...
        checked_proxies_unsuccessful += 1
        checked_proxies += 1
        return None, None
...
def send_request(url, proxies, proxy):
    try:
        ...
        logging.info(f"Checked proxies: {checked_proxies}, Used proxies: {len(used_proxies)}, Successful requests: {successful_requests}")
        user_agents[user_agent] += 1
        successful_request_list.append(url)
        return True
    except Exception as e:
...
def main():
    ...
    global specific_countries_proxies
    global country_specific_proxies
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        while successful_requests < max_successful_requests and len(proxies_http) + len(proxies_socks4) + len(proxies_socks5) > 0:
...
if finished:
    with open(f"{logDir}/successful_request.txt", "w") as f:
        f.write("Successful requests:\n")
        for request in successful_request_list:
            f.write(f"{request}\n")
    with open(f"{logDir}/Statistics_log.txt", "w") as f:
        f.write(f"Start time: {start_time}\n")
        f.write(f"End time: {datetime.now()}\n")
        f.write(f"Total proxies downloaded: {len(proxies_http) + len(proxies_socks4) + len(proxies_socks5)}\n")
        f.write(f"Total proxies used: {len(used_proxies)}\n")
        f.write(f"Total proxies checked successfully: {checked_proxies}\n")
        f.write(f"Total proxies checked unsuccessfully: {checked_proxies_unsuccessful}\n")
        f.write(f"Total proxies matched the specific countries: {specific_countries_proxies}\n")
        f.write(f"Total successful requests: {successful_requests}\n")
        f.write(f"Total unsuccessful requests: {len(used_proxies) - successful_requests}\n")
        f.write("User agent usage:\n")
